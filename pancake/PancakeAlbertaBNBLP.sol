...
// File contracts/alberta/PancakeAlbertaBNBLP.sol
...
contract PancakeAlbertaBNBLP is IAlberta, Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using Address for address payable;

    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

    IUniswapV2Factory public constant FACTORY = IUniswapV2Factory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73);
    ...
    function add(
        ...
        address otherToken = lpToken.token0() == WBNB ? lpToken.token1() : lpToken.token0();

        // Preparing BNB or farming tokens
        if (_paidToken != address(0) && _paidToken != otherToken) {
            IERC20(_paidToken).safeTransferFrom(msg.sender, address(this), _amountIn);

            address[] memory pathPaid = new address[](2);
            (pathPaid[0], pathPaid[1]) = (_paidToken, WBNB);
            IERC20(_paidToken).safeApprove(address(ROUTER), 0);
            IERC20(_paidToken).safeApprove(address(ROUTER), type(uint256).max);

            uint256 amountIn = IERC20(_paidToken).balanceOf(address(this));
            _safePriceImpactSwap(amountIn, pathPaid, _maxPriceImpact);
            ROUTER.swapExactTokensForETH(amountIn, 0, pathPaid, address(this), block.timestamp);
        } else if (_paidToken == otherToken) {
            IERC20(otherToken).safeTransferFrom(msg.sender, address(this), _amountIn);
        }

        // Compute the optimal amount of BNB and otherToken to be converted.
        uint256 bnbBalance = address(this).balance;
        uint256 swapAmt;
        bool isReversed;
        {
            (uint256 r0, uint256 r1, ) = lpToken.getReserves();
            (uint256 bnbReserve, uint256 otherReserve) = lpToken.token0() == WBNB ? (r0, r1) : (r1, r0);
        ...
        // Convert between BNB and farming tokens
        IERC20(otherToken).safeApprove(address(ROUTER), 0);
        IERC20(otherToken).safeApprove(address(ROUTER), type(uint256).max);
        address[] memory path = new address[](2);
        (path[0], path[1]) = isReversed ? (otherToken, WBNB) : (WBNB, otherToken);

        _safePriceImpactSwap(swapAmt, path, _maxPriceImpact);
        if (isReversed) {
            ROUTER.swapExactTokensForETH(swapAmt, 0, path, address(this), block.timestamp); // farming tokens to BNB
        } else {
            ROUTER.swapExactETHForTokens{value: swapAmt}(0, path, address(this), block.timestamp); // BNB to farming tokens
        }
    ...
    function remove(
        ...
        // Remove all liquidity back to BNB and farming tokens.
        lpToken.approve(address(ROUTER), type(uint256).max);
        ROUTER.removeLiquidityETH(otherToken, lpToken.balanceOf(address(this)), 0, 0, address(this), block.timestamp);

        // Convert farming tokens to BNB.
        address[] memory path = new address[](2);
        (path[0], path[1]) = (otherToken, WBNB);

        IERC20(otherToken).safeApprove(address(ROUTER), 0);
        IERC20(otherToken).safeApprove(address(ROUTER), type(uint256).max);

        uint256 amountIn = IERC20(otherToken).balanceOf(address(this));
        _safePriceImpactSwap(amountIn, path, _maxPriceImpact);
        ...
        if (_paidToken != address(0)) {
            ...
            (pathToPaid[0], pathToPaid[1]) = (WBNB, _paidToken);

            _safePriceImpactSwap(address(this).balance, pathToPaid, _maxPriceImpact);
            ROUTER.swapExactETHForTokens{value: address(this).balance}(0, pathToPaid, _to, block.timestamp);
        } else {
    ...
    function _safePriceImpactSwap(
        ...
        for (uint256 i; i < _path.length - 1; i++) {
            (uint256 rIn, uint256 rOut) = PancakeLibrary.getReserves(address(FACTORY), _path[i], _path[i + 1]);
            uint256 q = ROUTER.quote(_amountIn, rIn, rOut);
            uint256 out = PancakeLibrary.getAmountOut(_amountIn, rIn, rOut);
